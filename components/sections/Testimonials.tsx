...
export default function Testimonials() {
    ...
    if (loading) {
        return (
            <section id="testimonials" className="py-[120px] bg-[#C7B5F5]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {testimonials.map((testimonial) => (
                        <div key={testimonial.id} className="bg-white rounded-2xl shadow-lg p-6 text-center hover:-translate-y-1 transition-all duration-300">
                            <div className="w-16 h-16 rounded-full bg-[#EDE7FB] flex items-center justify-center mx-auto mb-3 text-[#C7B5F5] font-bold text-xl">
                                {testimonial.name?.[0] || 'U'}
                            </div>
                            <div className="flex justify-center gap-0.5 mb-3">
                                {[...Array(5)].map((_, i) => (
                                    <FaStar key={i} className={`text-lg ${i < testimonial.rating ? 'text-[#FBBF24]' : 'text-[#E8E5F3]'}`} />
                                ))}
                            </div>
